...
func (t Training) meanSpeed() float64 {
	...
	if t.Duration.Minutes() != 0 {
		return t.distance() / (t.Duration.Minutes() / float64(MinInHours))
	} else {
		return 0
	}
...
func (r Running) Calories() float64 {
	...
	return ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours) // переделать мб
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	message := InfoMessage{TrainingType: s.TrainingType, Duration: s.Duration, Distance: s.distance(), Speed: s.meanSpeed(), Calories: s.Calories()}
	return message
}
